...
def output_amplicon_walks(
    ...
    *,
    ...
    cycle_path = pathlib.Path(
        output_prefix + f"_amplicon{amplicon_idx + 1}_cycles.txt"
    )
    ...
    if (
        pc_output_option == OutputPCOptions.ALL
        and len(bp_graph.path_constraints) > 0
    ):
        ...
        pc_output_option == OutputPCOptions.LONGEST
        and len(bp_graph.longest_path_constraints) > 0
    ):
        ...
        for constraint_i, finalized_pc in enumerate(longest_path_constraints):
            ...
            write_path_constraint_to_file(
                constraint_i, finalized_pc, basic_pc.path, fp
            )
...
def write_path_constraint_to_file(
    idx: int,
    finalized_pc: FinalizedPathConstraint,
    path: Walk,
    fp: io.TextIOWrapper,
) -> None:
