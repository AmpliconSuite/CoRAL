...
def minimize_cycles(
    ...
    for seqi in range(lseg):
        ...
        m.addQConstr(cn_expr <= bp_graph.sequence_edges[seqi][-1])  # type: ignore[operator]
    for ci in range(lc):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + ci) * k + i]
        m.addQConstr(cn_expr <= bp_graph.concordant_edges[ci][-1])
    for di in range(ld):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + lc + di) * k + i]
        m.addQConstr(cn_expr <= bp_graph.discordant_edges[di][-1])
        m.addQConstr(cn_expr >= p_bp_cn * bp_graph.discordant_edges[di][-1])
    for srci in range(lsrc):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + lc + ld + 2 * srci) * k + i]
            cn_expr += w[i] * x[(lseg + lc + ld + 2 * srci) * k + k + i]
        m.addQConstr(cn_expr <= bp_graph.source_edges[srci][-1])
...
def maximize_weights_greedy(
    ...
    if next_w < resolution:
        ...
        logging.debug(
            "#TIME "
            + "%.4f\t" % (time.time() - state_provider.TSTART)
            + "\tCycles/paths explain sufficient CN in graph, greedy iteration completed.",
        )
    else:
        logging.debug(
            "#TIME "
...
def cycle_decomposition(
    ...
    for amplicon_idx in range(len(bb.lr_graph)):
        ...
        logging.info(
            "#TIME "
            + "%.4f\t" % (time.time() - state_provider.TSTART)
            + "Begin cycle decomposition for amplicon %d." % (amplicon_idx + 1),
        )
        logging.info(
            "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Total CN weights = %f." % total_weights,
        )
        ...
        logging.info(
            "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Total num initial cycles / paths = %d." % k,
        )
        ...
        while k <= nedges:
            ...
            if status_ == GRB.INFEASIBLE:
                logging.info(
                    "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Cycle decomposition is infeasible.",
                )
                logging.info(
                    "#TIME "
                    + "%.4f\t" % (time.time() - state_provider.TSTART)
...
def reconstruct_cycles(
    ...
    logging.info(
        "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Computed all subpath constraints.",
    )
    alpha_ = 0.01
    postprocess_ = 0
    nthreads = -1
    ...
    logging.info(
        "#TIME "
        + "%.4f\t" % (time.time() - state_provider.TSTART)
        + "Completed cycle decomposition for all amplicons.",
    )
