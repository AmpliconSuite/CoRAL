...
class BreakpointGraph:
    ...
    amplicon_intervals: dict[int, types.AmpliconInterval] = field(default_factory=dict)
    ...
    def compute_cn_sr_lr(
        ...
        logging.debug(
            "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Adjacent list for estimating CN:",
        )
        for node in self.nodes.keys():
            logging.debug(
                "#TIME "
        ...
        if nconstraints > 0:
            ...
            if sol["status"] == "optimal" or sol["status"] == "unknown":
                if sol["status"] == "optimal":
                    logging.debug(
                        "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Found optimal solution.",
                    )
                else:
                    logging.debug(
                        "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Reached maximum num iterations.",
                    )
                logging.debug(
                    "#TIME "
                    + "%.4f\t" % (time.time() - state_provider.TSTART)
                ...
                logging.debug(
                    "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "\tgap = %f" % (sol["gap"]),
                )
                logging.debug(
                    "#TIME "
                    + "%.4f\t" % (time.time() - state_provider.TSTART)
            ...
            logging.debug(
                "#TIME " + "%.4f\t" % (time.time() - state_provider.TSTART) + "Skipped convex optimization.",
            )
            for seqi in range(lseq):
