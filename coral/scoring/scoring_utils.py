...
def breakpoint_match(bp1: tuple, bp2: tuple, tolerance: int = 100):
...
def repartition_intervals(cycle, intervals):
    ...
    for _, interval in cycle.iterrows():
        ...
        segments = pyranges.PyRanges(intervals).intersect(pyranges.PyRanges(pd.DataFrame(interval).T))
        ...
        for _, segment in segments_df.iterrows():
            ...
            new_intervals.append([segment.Chromosome, segment.Start, segment.End, orientation, segment.segment_label])
        
    
    return pd.DataFrame(new_intervals, columns = ['Chromosome', 'Start', 'End', 'orientation', 'segment_label'])
...
def compute_normalized_longest_cycle_subsequence(true_cycle, reconstructed_cycles, _):
    ...
    for cycle in reconstructed_cycles["cycle_id"].unique():
        ...
        _true_cycle['segment'] = _true_cycle.apply(lambda x: (x.Chromosome, x.Start, x.End), axis=1)
    
        reconstructed_cycle = reconstructed_cycles[
            reconstructed_cycles["cycle_id"] == cycle
        ].copy()
        reconstructed_cycle['segment'] = reconstructed_cycle.apply(lambda x: (x.Chromosome, x.Start, x.End), axis=1)
        ...
        grs = {
            ...
            for n, s in zip(
                ["bins", "e1", "e2"],
                [pyranges.PyRanges(binned_genome), pyranges.PyRanges(_true_cycle), pyranges.PyRanges(reconstructed_cycle)],
            )
        }
        ...
        reconstructed_cycle_repartitioned = repartition_intervals(reconstructed_cycle, overlaps)        
        ...
        true_cycle_string = true_cycle_repartitioned.apply(lambda x: (x.segment_label, x.orientation), axis=1).values
        reconstructed_cycle_string_fw = reconstructed_cycle_repartitioned.apply(lambda x: (x.segment_label, x.orientation), axis=1).values
        reconstructed_cycle_string_rev = [(x[0], flip_orientation(x[1])) for x in reconstructed_cycle_string_fw[::-1]]
        
        max_lcs = 0

        for i in range(len(reconstructed_cycle_string_fw)):
            
            rotated_reconstruction = list(reconstructed_cycle_string_fw)[i:] + list(reconstructed_cycle_string_fw)[:i]
            common_subsequence_length = find_longest_subsequence(true_cycle_string, rotated_reconstruction, overlaps[['segment_label', 'interval_length']].set_index('segment_label')['interval_length'].to_dict())
            if common_subsequence_length > max_lcs:
                max_lcs = common_subsequence_length
            
        for i in range(len(reconstructed_cycle_string_rev)):
            
            rotated_reconstruction = list(reconstructed_cycle_string_rev)[i:] + list(reconstructed_cycle_string_rev)[:i]
            common_subsequence_length = find_longest_subsequence(true_cycle_string, rotated_reconstruction, overlaps[['segment_label', 'interval_length']].set_index('segment_label')['interval_length'].to_dict())
            if common_subsequence_length > max_lcs:
                max_lcs = common_subsequence_length
                
        if (max_lcs / total_length) > overall_max_normalized_lcs:
            overall_max_lcs, overall_max_normalized_lcs = max_lcs, (max_lcs / total_length)
