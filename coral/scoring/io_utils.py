...
def read_cycles_file_to_bed(input_file, num_cycles=None):
    ...
    bed_df = pd.DataFrame(columns=["Chromosome", "Start", "End", "orientation", "cycle_id", "iscyclic", "weight"])
    _num_cycles = len(cycles) if num_cycles is None else num_cycles
    for i in range(1, _num_cycles + 1):
        for seg in cycles[i][2]:
            new_row = pd.DataFrame([[seg[0], seg[1], seg[2], seg[3], i, cycles[i][0], cycles[i][1]]], columns = bed_df.columns)
    ...
    with open(file, "r") as f:
        ...
        for line in f:
            if line.startswith("sequence"):
                ...
                left_chrom, left_pos = left.split(":")[0], int(
                    left.split(":")[1].split("-")[0]
                )
                right_chrom, right_pos = right.split(":")[0], int(
            ...
            if line.startswith("discordant"):
                ...
                if left_chrom > right_chrom:
                    breakpoint_edges.append([edge_class, right_chrom, right_pos, right_orient, left_chrom, left_pos, left_orient, copycount, read_pairs.strip()])
                elif left_chrom == right_chrom and left_pos > right_pos:
                    breakpoint_edges.append([edge_class, right_chrom, right_pos, right_orient, left_chrom, left_pos, left_orient, copycount, read_pairs.strip()])
                else:
                    breakpoint_edges.append([edge_class, left_chrom, left_pos, left_orient, right_chrom, right_pos, right_orient, copycount, read_pairs.strip()])
...
def bin_genome(true_segments, reconstructed_segments, margin_size=10000):
    ...
    df_bins = pd.DataFrame(np.concatenate((true_segments[['Chromosome', 'Start']].values,
    ...
    df_bins = df_bins[(df_bins['Chromosome'] == df_bins["Chromosome2"]) & (abs(df_bins['Start'] - df_bins['End']) >= 0)]
...
def get_seq_edges_from_decoil(results, base_coverage=13.0):
    ...
    for _, entry in results.iterrows():
        sequence_edges.append([entry.Chromosome, entry.Start, entry.Chromosome, entry.End, (entry.End-entry.Start), entry.weight])
    ...
    for _, cycle_df in results.groupby('cycle_id'):
        ...
        for i in range(len(cycle_df)):
            ...
            left_chrom, left_start, left_end, left_orient = entry_i[['Chromosome', 'Start', 'End', 'orientation']]
            right_chrom, right_start, right_end, right_orient = entry_j[['Chromosome', 'Start', 'End', 'orientation']]

            right_pos = right_start if right_orient == '+' else right_end
            left_pos = left_end if left_orient == '+' else left_start

            if left_chrom > right_chrom:
                breakpoint_edges.append(['discordant', right_chrom, right_pos, flip_orient(right_orient), left_chrom, left_pos, left_orient])
            elif left_chrom == right_chrom and left_pos > right_pos:
                breakpoint_edges.append(['discordant', right_chrom, right_pos, flip_orient(right_orient), left_chrom, left_pos, left_orient])
            else:
                breakpoint_edges.append(['discordant', left_chrom, left_pos, left_orient, right_chrom, right_pos, flip_orient(right_orient)])
