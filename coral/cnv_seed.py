...
def parse_centromere_arms() -> dict[str, ChrArmInfo]:
    ...
    with open(
        ...
        while p_line:
            ...
            if p_intv.chr_tag != q_intv.chr_tag:
                raise ValueError("Centromere file is not sorted by chromosome.")

            full_intv = Interval(p_intv.chr_tag, p_intv.start, q_intv.end)
            chr_arms[p_intv.chr_tag] = ChrArmInfo(
                interval=full_intv,
                p_arm=SingleArmInfo(p_intv, size=p_intv.end),
                q_arm=SingleArmInfo(
                    q_intv, size=CHR_SIZES[p_intv.chr_tag] - q_intv.end
                ),
            )
            p_line = fp.readline()
...
def run_seeding(
    ...
    for line in cn_seg_file:
        ...
        if s[0] != "chromosome":
            ...
            if cn >= gain and (end <= arm_intv.start or start >= arm_intv.end):  # type: ignore[possibly-undefined]
                if (
                    len(cur_seed) > 0
                    and chr_tag == cur_seed[-1].chr_tag
    ...
    with open(output_filename, "w") as fp:
        for seed_intvs in cnv_seeds:
            ...
            seed_chr_tag = seed_intvs[-1].chr_tag
            ...
            if len(seed_intvs) > 0:
                ...
                for cns in seed_intvs:
                    if lastseg and cns.start - lastseg.end <= max_seg_gap:
                        ...
                        fp.write(
                            f"{lastseg.chr_tag}\t{lastseg.start}\t{lastseg.end-1}\n"
                        )
                        sum_seed_len = 0
                        lastseg = cns
